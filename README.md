...
後半段有些 recipe 會直接讀前幾天的目錄。要單獨複製的話，複製完先跑一次 `verify.sh`，
它會告訴你少了什麼。
...
- **`verify.sh`**：重跑對照並檢查判準。環境不足、或有步驟被跳過，它不會當成通過
...
| 05 | [innerHTML 的假通過](recipes/05-innerhtml-fake-green/) | [Day 5](https://ithelp.ithome.com.tw/articles/10401465) | **可以跑了** |
...
| 17 | [模型講的話變成動作，中間那道檢查](recipes/17-words-into-actions/) | [Day 17](https://ithelp.ithome.com.tw/articles/10403142) | **可以跑了** |
| 18 | [四道檢查，跟它們各自防的是什麼](recipes/18-not-a-free-chatgpt/) | [Day 18](https://ithelp.ithome.com.tw/articles/10403409) | **可以跑了** |
...
| 24 | [全部通過是防住了還是沒打到](recipes/24-green-or-never-hit/) | [Day 24](https://ithelp.ithome.com.tw/articles/10404722) | **可以跑了** |
...
| 28 | [那張表是拿什麼跑出來的](recipes/28-what-was-it-run-with/) | Day 28 | **可以跑了**（對帳本身不用模型） |
| 29– | 隨連載加入 | Day 29–30 | 未完成 |

每一份都是 `bash recipes/<名字>/verify.sh`。一次跑全部：

```bash
for d in recipes/*/; do bash "$d/verify.sh" >/dev/null || echo "沒過：$d"; done
```

**這張表由 `bash check-index.sh` 盯著**，少一列或指到不存在的資料夾就會失敗。
最新那份 recipe 的 `verify.sh` 最後一條會呼叫它，所以每天都會跑到。
會有這個檢查，是因為這張表在 8/16 以前停在 06 停了十二天：
每天都有人跑 verify，沒有人重讀 README。

跑得動的前提：07 之後要 Node 20 以上，14、15、17、18 另外要 `python3`（洗牌與突變用）。
缺什麼它會直接講再退出，不會安安靜靜地當作通過。打真模型是選配，
...
只跑會通過的那一邊沒有價值，因為它證明不了洞是真的。

想當練習題的話，挑一份有 `before/` 的（01、05、09、16），把那個漏洞補起來再跑一次。
看的是**判定有沒有改變**，不是數字變小：`verify.sh` 期望的是「有洞那份真的有洞」，
所以你補好之後它會說沒過，那就是你改對了。

其餘二十三份沒有 `before/`，因為它們比的不是兩份程式碼：可能是兩種檢查方式、
兩種跑法，或同一支程式在兩個環境下的結果。要拿哪一份當練習題，先讀那份 README 的開頭。

## 授權

授權是 MIT，把 recipe 複製進自己的專案不用問。只有一件事要注意：有幾份會讀別的目錄，複製完跑一次 `verify.sh` 就知道少了什麼。
