...
| 情況 | verify.sh 的反應 |
|---|---|
| 什麼都有 | 通過 74／沒過 0／沒有結論 0，結束碼 0 |
| 沒有 `node` | 通過 1／沒過 0／沒有結論 7，結束碼 1 |
| 沒有 `npx` | 通過 65／沒過 0／沒有結論 3，結束碼 1 |
| 沒有 `curl` | 通過 63／沒過 0／沒有結論 1，結束碼 1 |
| `curl` 在，但問不到註冊處 | 通過 67／沒過 0／沒有結論 1，結束碼 1 |

（2026-08-08 重測。缺工具那幾列的量法是把 `PATH` 上所有可執行檔連到一個暫存目錄、
只少掉指定那一支，這樣 `command -v` 才真的找不到它。用「放一支回傳 127 的假工具」
量會得到一堆沒過，那是量法的錯不是降級壞了。）

沒有任何一種缺法會判沒過：**沒過的意思是你要驗的東西壞了，不是這台機器不適用。**
反過來說沒有結論也不算通過，所以它把結束碼推到 1。
`bash verify.sh && echo 通過` 不該在一台連 `node` 都沒有的機器上印出那句話。

四種缺法下都還過得了的只有第 1 節那條把 `PATH` 清空、看 `inventory.sh` 會不會當場停在
結束碼 2 的檢查。它不需要任何工具在，所以少了什麼都驗得到。

打錯節號會直接停：`bash verify.sh 9` 印「沒有第 9 節」、結束碼 2。
不擋的話那會是 通過 0、沒過 0、沒有結論 0，然後結束碼 0，也就是這一份最想抓的那種假通過。
`bash mutations.sh 99` 同理，印「一種突變都沒跑到。沒有第 99 種，可用的是 1 到 44」、結束碼 2。
...
| `probe.sh` | 這條流程那一步。自己造正反兩組，寬範圍要讀到、收窄後要讀不到 |
...
| `mutations.sh` | **故意把腳本弄壞四十一種，看 `verify.sh` 會不會判沒過。** 有一種沒抓到就是假通過 |
...
畫面上看不出來，`mutations.sh` 有兩條就是在驗這件事：拿掉一類樣式，兩個入口要各自沒過。
...
`verify.sh` 第 7 節在盯上面這四條。`demo/` 爛掉的話那一節會沒過，
...
於是第 3 步會「通過」。** 那是一次假通過，而它跟真的降權成功在畫面上長得一樣。
...
`Access denied - path outside allowed directories` 這句原文，上游改掉的話它會沒過，
因為 README 引了它。
