...
# 編號打錯的話每一種都被跳過，收尾算出「抓到 0 種 / 漏掉 0 種」然後離開碼 0。
...
run_case '總計的字元數另外算一個'              2 mcp-rpc.cjs \
  's{"\\t" \+ total \+ "\\n"}{"\\t" + (total + 7) + "\\n"}'
run_case 'server 起不來也回離開碼 0'          2 list-tools.sh \
...
run_case '探針壞了跟降權沒生效共用同一個離開碼' 4 probe.sh \
  's{先修環境（npx 抓不到套件、允許目錄給錯、檔案沒建起來都會走到這裡）。離開碼 3。\\n.\n  exit 3}{先修環境。離開碼 1。\\n\x27\n  exit 1}'
