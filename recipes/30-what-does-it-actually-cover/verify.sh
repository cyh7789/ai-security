...
# 另一半是那兩份判準檔有沒有偷偷把東西藏起來，底下第 3、5、6 條守的是那一半。
#
...
while IFS="$(printf '\t')" read -r g expand src rerun sign why; do
  ...
  case "$rerun" in
    ...
    *)
      # 反過來也要守：有重跑指令的列不准簽成沒測，那等於寫了指令卻沒跑。
      printf '%s' "$sign" | grep -qE '^0/0/[0-9]+$' \
        && { bad "「$g」有重跑指令（${rerun}）卻簽成沒測"; M=1; } ;;
  esac
done <<EOF
$(rows "$STD")
EOF
[ "$M" = 0 ] && ok "有指令的都有結果，沒指令的都算沒測"
fi
