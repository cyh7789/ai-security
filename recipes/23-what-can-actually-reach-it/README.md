...
這一欄仍然由人維護，但填進去之前要有實跑：`reach.sh` 把跑得動的路徑各跑一次，`verify.sh` 拿 `reach.log` 逐列對帳，對不上就紅；跑不動的列必須明寫為什麼量不到。判準來自 Day 22 的教訓：那天我把「我沒找到關工具的旗標」寫成「它關不掉」。`沒驗過` 跟 `被擋死` 是同一組區別。

`reach.sh` 量的是「這條路徑上的內容停在哪一層」，不是「模型會不會照做」。罐頭模型不會被說服；模型照不照做要打真模型，那是 Day 24 攻擊變體的事。

## 量出來的

十五列：八條到得了、四條被擋死、三條沒驗過。**「到得了」只回答「這一列量到的那個終點抵達了沒」，八列的終點不是同一級**：R4／R6／R14 到的是真的改狀態的動作，R10 到的是真的發出去的網路請求，R2／R3 只到交付邊界（過了四道檢查，這份示範沒有寄送這件事），R12 只到模型，R1 是正常流量。被擋死那四條裡有兩條是同一條路徑換一道閘的對照（R5 對 R4、R13 對 R10），留著是為了說明那個洞不是無解；另外兩條（R8、R9）不是對照，是本來就過不去。
