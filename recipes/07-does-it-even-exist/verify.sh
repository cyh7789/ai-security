...
# 節號打錯的話，下面每一節都不跑，收尾算出通過 0、沒過 0、沒有結論 0，然後離開碼 0。
# 一份講假通過的東西，最不能留的就是這種形狀。
case "${ONLY}" in
  ''|1|2|3|4|5) ;;
...
# kill 寫在「起得來」那條分支裡是不夠的：起不來或斷言沒過就走另一條路，
...
# 後者是要驗的東西壞了。折成同一個旗標的話，沒裝 node 的機器會看到滿畫面沒過。
...
# check-pkgs.sh 對那台的處理是大聲降級不是停下來，所以這支不會因為它掛掉而整片沒過。
  ...
  if [ -n "${M}" ]; then
    ...
    if [ "${HAVE_NET}" = 0 ]; then
      ...
      if [ -z "${P_OK}" ] || [ -z "${P_ONE}" ] || [ -z "${P_YES}" ] \
        ...
        # 而且是「那台正常」那一輪沒有的話。收尾整段被拿掉就會沒過，因為那幾輪的說明
        ...
        # 都代表它們在畫面上分不開。
        # 寫死一句話去比對的版本兩個方向都會壞：只改措辭會誤報，
        # 用新措辭把 bug 原封不動種回去會假通過（兩個方向都實測到過）。
...
sect "2 三種輸入方式（參數、檔案、剪貼簿）給同一個答案"
  ...
  if [ -n "${M}" ]; then
    ...
    # 三邊就不一致了，訊息指向輸入解析，真正的原因卻是那台在擋（8/07 實測撞到）。
...
sect "3 停更的套件還是有人在下載，所以下載數不是活著的證明"
  ...
  if [ -n "${M}" ]; then
    ...
    if [ -z "${dl}" ] || [ -z "${mod}" ]; then
      ...
      # 把沒問到判成沒過，就是這一份從第一節開始在反對的那個混淆。
      skip "下載數這一格是「${dl}」，沒問到就沒得比。這不是沒過，是這一輪沒驗到"
...
${out}"
  ...
  if [ -n "${M}" ]; then
    ...
    # npm 直接吐設定錯誤而不是連線錯誤，這條檢查就永遠沒過，而且理由是我的旗標，
    # 不是被測的東西。兩個一起給。
    ...
    if [ "${rc}" = 0 ]; then
      ...
      # README 對外寫的是「離開碼一律是 2」，只判非零的話改成 exit 1 也抓不到
    ...
    # 而且要比對到 code 那一行：光有 `npm error` 三個字，罐頭的「你是不是在代理後面」
...
EOT
    ...
    # 「檔不在」證明的就只是這顆探針從頭到尾都不會寫檔，那條斷言永遠會過。
    ...
    if [ -e "${MARK_CTRL}" ]; then
      ...
      bad "正向對照沒跑出痕跡（rc=${rcc}），不帶旗標那一輪也沒寫出檔，那下面那條「檔不在」永遠會過：$(printf '%s' "${outc}" | tail -3)"
    fi
