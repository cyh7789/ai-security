...
# 兩件事這裡刻意做得比「看離開碼」嚴：
#
# 一、「沒有結論」不算「沒過」。混在一起的話，一台沒有模型的機器會把整張表
#    印成十條通過，而其實一條都沒驗到（2026-08-25 實測）。
# 二、每一列要指定**哪幾條**該沒過，而且是精確比對。只看顏色的話，一個突變
#    可以靠別的檢查沒過來冒充，而它宣稱要驗的那條其實壞掉也沒人知道：把第 10 條的
