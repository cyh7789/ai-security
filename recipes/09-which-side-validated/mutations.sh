#!/usr/bin/env bash
# verify.sh 全部通過只證明現在這份是對的，不證明它在事情壞掉的時候會沒過。
# 這支跑十一種變化：十種是故障，必須讓 verify.sh 沒過；
# 一種是共用來源的反向對照，它必須照舊通過，不然那個共用只是宣稱。
...
run() { # run <說明> <sed 表達式> <目標檔>
  ...
  # sed 沒套進去的時候整條會靜默變成「跑原版」，畫面上長得跟「檢查漏掉」一模一樣。
  # 2026-08-09 就是這樣誤判了一次：BSD sed 吃不下多行表達式，我讀成檢查有洞。
  if cmp -s "./$3" "$work/$3"; then
    MISSED=$((MISSED+1)); printf '  \033[31m壞了\033[0m %s（突變沒套用進去，這條什麼都沒驗到）\n' "$1"
    rm -rf "$work"; return
  fi
...
p.write_text(s)" "$work/$2"
  ...
  # 第四個參數是 NEG 的時候，期望反過來：這種改動「不該」讓 verify 沒過。
  if bash "$work/verify.sh" > /dev/null 2>&1; then
    if [ "${4:-}" = NEG ]; then
      HELD=$((HELD+1)); printf '  \033[32m照舊通過\033[0m %s\n' "$1"
    else
      MISSED=$((MISSED+1)); printf '  \033[31m漏掉\033[0m %s\n' "$1"
    fi
  else
    if [ "${4:-}" = NEG ]; then
      MISSED=$((MISSED+1)); printf '  \033[31m不該沒過\033[0m %s\n' "$1"
      bash "$work/verify.sh" 2>&1 | grep '沒過' | head -2
    else
      CAUGHT=$((CAUGHT+1)); printf '  \033[32m抓到\033[0m %s\n' "$1"
    fi
  fi
...
# 專門打 probe-table：verify.sh 過不過不算數，要看那張表印出什麼。
...
runpy "after 把界線改成 1 到 20（表單會跟著變，所以不該沒過）" after/rules.mjs '
...
echo
printf '════ 故障 %s 種沒過、反向對照 %s 種照舊通過，不符 %s 種 ════\n' "$CAUGHT" "$HELD" "$MISSED"
[ "$MISSED" -eq 0 ]
