...
bash mutations.sh           # 十一種變化：十種該沒過，一種反向對照該照舊通過
```
...
拿表單上的 `max` 去打，那個值要收、`max+1` 要擋。兩台現在都會過，因為 `before` 的兩份
數字剛好還一樣。**「剛好一樣」跟「不會走散」是兩件事**，差別要靠突變才看得出來：
改 `after/rules.mjs` 的界線，表單會跟著變，檢查照樣通過；改 `before/server.mjs` 的界線，
表單那份不會跟，檢查就沒過。

## probe-table.sh 為什麼要讀回來

判定「收不收」不看狀態碼，看值。第一版是看狀態碼的，那是一次假通過：
...
```
通過 擋下來的原因是值域檢查，不是伺服器壞了
通過 被擋之後那筆沒有被改動（2）
通過 合法的 quantity=3 仍然改得動（3）
通過 合法的 quantity=1 仍然改得動（1）
通過 合法的 quantity=10 仍然改得動（10）
```

第一條擋掉「它其實是 500，只是剛好也沒改成」。後面三條是正對照：
把所有請求都拒絕掉，前面每一條都會變成通過，而那是一個壞掉的 API 不是一個安全的 API。
...
全部通過只代表現在這份是對的，不代表它在事情壞掉的時候會沒過。十一種變化，其中十種是故障、必須沒過，一種是反向對照、必須照舊通過：

| 壞法 | 應該沒過的原因 |
|---|---|
| `after` 的 PATCH 把驗證拿掉 | 洞又回來了 |
| `after` 改成什麼都擋 | 正對照那條會沒過，這是「全擋」偽裝成「安全」 |
...
| `after` 的 POST 一律拒絕 | POST 那邊的正對照會沒過 |
| **`after` 的界線改成 1 到 20** | **這一條反過來，它不該沒過**：表單跟端點都問 `rules.mjs`，一起變就還是一致 |
| `before` 只改端點界線 | 表單那份沒跟上，宣告與實際切的位置走散 |
| 說謊的 PATCH（打 `probe-table`） | 那張表要印「值進去了」，不是靠 400 就說擋住了 |

十種故障都要沒過、那一種反向對照要照舊通過，任何一項不符就代表 verify.sh 有一格是假的。

反向那條自己也要能翻臉：把 `after` 的表單改回自己寫死（共用斷掉），它必須印「不該沒過」。
不會翻臉的反向對照，跟一個永遠印通過的假檢查沒有分別。

**有一種故意沒放進來。** 把 `after` 兩支端點各自抄一份行內檢查，
行為跟現在這份完全相同，`verify.sh` 一條都不會沒過，而且永遠不會。
