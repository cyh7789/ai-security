...
//
// 這支只做方向性的 sanity check，不把輸入條數當成正確群數：
// 五條輸入完全可能共用同一種理由，也可能各自生出好幾種。
// 要看的只有一件事：字面方法會不會把明顯相關的理由炸成數十堆。
//
// 還有一個範圍限制要記住：分群是在每個組**各自**跑的，不是把 203 種
// 全部放進同一張圖。所以各組堆數相加不等於「整批一起分群的結果」，
// 跨組的邊沒有被算進去（8/19 外審抓到，正文原本拿相加的總數在說事）。
...
console.log("注意：輸入條數不是正確群數，這裡只看字面方法會不會把明顯相關的理由炸成數十堆。");
console.log(`${e[0]} 那組只有 ${e[1]} 條輸入，理由仍堆成 ${e[3]} 堆，字面分群在這批資料上壓不動。`);
