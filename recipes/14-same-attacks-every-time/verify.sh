...
if run 2; then
  ...
  # 其餘六種載體各自的條數也要對：dom 2、http 2、kb 1、tool 1、data 1、turns 1。
  # 寫成一個「其他 8 條」的常數的話，多一條 tool 少一條 dom 也會通過。
  G_REST=""
  for c in dom:2 http:2 kb:1 tool:1 data:1 turns:1; do
    n=$(grep -c "\"carrier\":\"${c%%:*}\"" attacks.jsonl)
    [ "${n}" = "${c##*:}" ] || G_REST="${G_REST} ${c%%:*}=${n}(要${c##*:})"
  done
  [ "${G_IN}" = "${W_IN}" ] && [ "${G_PG}" = "${W_PG}" ] && [ -z "${G_REST}" ] \
    && [ "${G_ALL}" = "$((W_IN + W_PG + 8))" ] \
    && ok "input ${G_IN}、page ${G_PG}、dom/http/kb/tool/data/turns 各 2/2/1/1/1/1，合計 ${G_ALL}" \
