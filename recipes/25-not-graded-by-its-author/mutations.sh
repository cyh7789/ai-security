...
bite() {  # $1=描述 $2=預期抓到的條號 $3.. = sub 的參數
  ...
  restore
  # 只認離開碼 1。離開碼 2 是「這一跑沒有結論」，算成抓到的話，
  # 一個讓 verify.sh 開頭就死掉的突變會被記成成功。
  if [ "$rc" = 1 ]; then
    local hit
    hit=$(printf '%s' "${out}" | awk -v w="${want}" '/^=== /{n=$2} /^  沒過/{print n}' | sort -u | tr '\n' ' ')
    case " ${hit} " in
      *" ${want} "*) printf '  抓到\t%-44s 第 %s 條\n' "${desc}" "${want}"; G=$((G+1)) ;;
      *) printf '  抓錯\t%-44s 預期第 %s 條，沒過的是：%s\n' "${desc}" "${want}" "${hit}"; B=$((B+1)) ;;
    esac
  else
    printf '  沒抓到\t%-44s 離開碼 %s\n' "${desc}" "${rc}"; B=$((B+1))
  fi
}
...
# 抓得到的原因：第 20 條會造一個深度 100 的複本，那種複本 is-shallow 是 true
...
# 抓的是祖先關係那一支，不是「這個 SHA 在不在版控裡」：塞一個不成立的字串進去，
# 沒過的會是後者，而 README 把第 5 條列為對抗「順序偷偷倒過來」的防法，
# 那個防法真正的機制是 merge-base --is-ancestor。
# 改 final 的而不是 before 的：before-default 與 before-gate 的 commit 還被第 1、3 條
# 拿去重跑，動它們會同時抓到兩條，而 bite 只認一條。
...
bite "把不修的那條也塗成通過" 12 final/run.tsv \
  'C07\tR10\t擋\t沒擋\t沒擋\t缺口' 'C07\tR10\t擋\t擋住\t擋住\t符合'

echo
echo "=== 反向控制：不影響行為的改動，要維持通過 ==="
