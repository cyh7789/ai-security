...
case_ "9 直接問它組合，它吐的那兩行靜態引用都接不起來"
# 前兩輪是逐檔問與逐類問，兩種問法都預設答案落在單一位置上，
# 所以「它看不到組合」有可能是問法決定的。這一條守的是把那個變因拿掉的那一輪。
CA=chain-ask/answer.txt
if ! python3 -c "pass" >/dev/null 2>&1; then
  # 這一節要數引用邊。沒有 python3 是環境不到位，照 Day 22 那份公約算沒有結論，
  # 不是沒過（外部審查實測：假的 python3 讓這節印出「邊是：無」判沒過，那是誤報）。
  skip "沒有 python3，數不了引用邊，這一條驗不掉"
elif [ ! -r "$CA" ]; then
  ...
  [ "$M" = 0 ] && ok "那兩行的靜態引用都接不起來、唯一那條它沒指出來，同一問法重跑十次逐位元相同"
fi
