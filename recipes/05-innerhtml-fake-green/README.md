...
開著的瀏覽器裡也驗得掉。整支跑在 `mktemp -d` 開的暫存目錄裡，不碰你的檔案。
...
所以那個通過跟修法一點關係都沒有，它在修之前就會過。

## 打哪裡：送進 `answer`，不是打聊天輸入框

這一格要先講，它是最容易讓整個測試變成假通過的地方。
...
「這次沒跳，大概是模型幫我包起來了」就會變成一個聽起來合理的解釋，而那又是一次假通過。
...
要看的不是 `after/` 那一列全部通過，是 **script 那一欄上下一樣**。一條在有洞與修好上
表現相同的檢查分不出這兩種狀態，那它就不是一個驗證。

第 5 節就在斷言這件事：script 那條在兩版必須相同，img 那條在兩版必須不同。
拿掉任何一邊，這支腳本就退化成「跑一次看它過不過」。
...
還是又拿到一個跟第二條一樣的通過。**它證明不了你的程式碼有沒有問題，
但它證明得了你的判準有沒有辨識力。**
...
**兩條輸入不是一組完整的 XSS 攻擊集。** 它們是這條檢查流程的起點，用途是證明你的驗證
