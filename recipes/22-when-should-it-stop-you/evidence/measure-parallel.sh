#!/usr/bin/env bash
# 21 那支會暫時改 18 的 gates.mjs。平行跑 18/20/21 會不會讓 18、20 誤報？
# 序列跑 N 次當對照，平行跑 N 次，比兩邊沒過幾次。
# 只有一邊沒過才算數：兩邊都沒過代表是別的原因，跟平行無關。
set -u
cd ../..
N="${1:-10}"
OUT=./parallel.tsv
...
awk -F'\t' 'NR>1{n[$1]++; f18[$1]+=($3!=0); f20[$1]+=($4!=0); f21[$1]+=($5!=0)}
  END{for(k in n) printf "%s\t%d 輪\t18 沒過 %d\t20 沒過 %d\t21 沒過 %d\n", k, n[k], f18[k], f20[k], f21[k]}' "${OUT}"
