...
#
# 這一支改的是 C04 走哪一道檢查：從擋得住的外部基準檢查換成擋不住的意圖核對檢查。
# 那條案例的紀錄寫著「擋住」，所以它會變成「退步了」。
# 判準是 node --test 的離開碼：斷言寫成排除式的時候，那邊照樣是 13 pass 離開碼 0，
# 而現在它必須沒過。這一段是文章引用的那個現象，落成腳本才有人能重跑。
#
# 離開碼：0 測試如預期沒過、1 測試沒抓到、2 環境不到位
set -u
cd "$(dirname "$0")"
export LC_ALL=C
...
# 這裡只要確認測試入口在動手之前會過就夠了。
node --test cases.test.mjs >/dev/null 2>&1 || { echo "動手之前 node --test 就沒過，這一跑沒有結論"; exit 2; }

# C04 原本走外部基準檢查（擋得住），換成意圖核對檢查（擋不住）。
python3 -c '
import io,sys
...
if [ "${RC}" != 0 ]; then
  echo "測試沒過，這就是正面表列的斷言該有的反應"
else
  echo "測試沒抓到。斷言又退化成排除式了，過幾條都不算數。"
  exit 1
fi
