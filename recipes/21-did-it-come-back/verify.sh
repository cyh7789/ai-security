...
#
# 這支會暫時改 recipe 18 的 gates.mjs（第 3、4 條要證明測試真的會沒過），
...
PY
  # 逐條記下來是誰沒過。只收整體退出碼的話，「這次是哪幾條沒過」在輸出裡沒有證據，
  # 而 d1 從頭到尾沒有真的失敗過這件事就看不出來（8/20 收斂後那輪抓到）。
  # 不要用 awk：macOS 的 awk 比中文欄位值會每一列都成立（recipe 19、20 各撞過一次），
  # 這裡用它會印出「沒過的是 id B4 d1 b1」，連表頭都算進去。
  RED=$(node regress.mjs 2>/dev/null | grep -E "^(B4|d1|b1)${TAB}.*${TAB}沒過$" | cut -f1 | tr '\n' ' ')
  restore
  [ -n "${RED}" ] && printf '        沒過的是：%s\n' "${RED}"
  [ -z "${RED}" ]
}
...
# d1 命中「冒充」跟「誘導」兩個詞，是 OR 關係：只拿掉一個它照樣被擋。
# 要讓它沒過得兩個都拿掉（8/20 外審指出 d1 沒抓到過，它建議的「拿掉冒充」實測不成立）。
BLACK_NO_D1='export const OUT_OF_SCOPE = ["詐", "假冒", "偽裝成", "套出"];'
...
# ── 1 三條現在全部通過 ────────────────────────────────
if want 1; then
  case_ "1 三條回歸測試在現行判準下全部通過"
  # 兩類分開問，不要從一個離開碼反推。8/22 踩過：這裡原本讀 rc=2 當
  # 「缺口樁動了」，而 regress.mjs 早就改成只回 0／1，結果只有 b1 沒過的時候
  # 它報「防線沒過：」而冒號後面是空的，指控的正是沒事的那一邊。
  ...
  if [ "${LINE_RC}" != 0 ]; then
    bad "防線沒過：$(grep -E "^(B4|d1)${TAB}.*沒過$" /tmp/r21.out | cut -f1,5 | tr '\n' ' ')"
...
# ── 3 把修補拿掉，測試會沒過（黑名單那一半）──────────────
# 規格第 4 步：故意把修補拿掉確認它會沒過。沒有真的失敗過的測試不算數。
if want 3; then
  case_ "3 「騙」加回黑名單，測試沒過"
  if break_and_run "${BLACK_NOW}" "${BLACK_OLD}"; then
    bad "「騙」加回去之後測試竟然還會過：這三條斷言接不到 gates.mjs 的行為"
  else
    ok "加回去就沒過，還原之後通過"
  fi
fi

# ── 4 把修補拿掉，測試會沒過（場景清單那一半）────────────
# 這一半是 8/20 當天差點漏掉的：只拿掉「騙」B4 還是 deny，
# 它改成卡在允許清單。修補有兩層，測試就要兩層都抓得到。
if want 4; then
  case_ "4 「公告」從場景清單拿掉，測試沒過"
  if break_and_run "${SCEN_NOW}" "${SCEN_OLD}"; then
    bad "拿掉「公告」之後測試還會過：B4 那條沒有真的在問「它會不會過」"
  else
    ok "拿掉就沒過，還原之後通過"
  fi
fi

# ── 9 d1 也要沒過過：拿掉它命中的那兩個詞 ──────────────────
# 「沒有真的失敗過的測試不算數」這條紀律，前面兩個突變只證明了 B4 跟 b1。
# d1 期望 deny，而那兩次都是把判準改嚴，改嚴不會讓它沒過，所以它一直沒被證明接得上。
# 這一條補上：拿掉它命中的「冒充」與「誘導」，它該沒過。
# 8/20 外審抓到，它建議只拿掉「冒充」，實測不成立（還有「誘導」擋著）。
if want 9; then
  case_ "9 拿掉 d1 命中的那兩個詞，它沒過"
  if break_and_run "${BLACK_NOW}" "${BLACK_NO_D1}"; then
    bad "兩個詞都拿掉了 d1 竟然還是通過：它沒有真的在問「這句話會不會被擋」"
  else
    ok "兩個詞都拿掉就沒過，還原之後通過"
  fi
fi
...
# 它的期望是 allow。哪天有人「順手把它改成 deny 比較安全」，這條會擋下來：
# 改判決方向要的是重新做那個取捨（誤擋跟缺口哪個貴），不是改一個筆誤。
if want 5; then
...
# ── 7 形狀 vs 行為那支自己會判 ────────────────────────
if want 7; then
  case_ "7 形狀斷言通過、行為斷言沒過"
  if bash shape-vs-behavior.sh > /tmp/svb21.out 2>&1; then
    ok "$(tail -1 /tmp/svb21.out)"
  else
    bad "shape-vs-behavior.sh 判沒過：$(tail -2 /tmp/svb21.out | tr '\n' ' ')"
  fi
fi

# ── 8 跑完之後 gates.mjs 回到原樣 ───────────────────
# 這支動了別人的判準。示範跑完把別人那道檢查留在改過的狀態，是最糟的失敗。
if want 8; then
