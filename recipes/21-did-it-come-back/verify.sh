...
# 要讓它紅得兩個都拿掉（8/20 外審指出 d1 沒紅過，它建議的「拿掉冒充」實測不成立）。
...
# ── 9 d1 也要紅過：拿掉它命中的那兩個詞 ──────────────────
# 「沒紅過的測試不算數」這條紀律，前面兩個突變只證明了 B4 跟 b1。
# d1 期望 deny，而那兩次都是把判準改嚴，改嚴不會讓它轉紅，所以它一直沒被證明接得上。
# 這一條補上：拿掉它命中的「冒充」與「誘導」，它該紅。
# 8/20 外審抓到，它建議只拿掉「冒充」，實測不成立（還有「誘導」擋著）。
if want 9; then
  case_ "9 拿掉 d1 命中的那兩個詞，它轉紅"
  if break_and_run "${BLACK_NOW}" "${BLACK_NO_D1}"; then
    bad "兩個詞都拿掉了 d1 竟然還是綠：它沒有真的在問「這句話會不會被擋」"
  else
    ok "兩個詞都拿掉就紅，還原之後綠"
  fi
fi
