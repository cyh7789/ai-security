...
對應 [Day 13](https://ithelp.ithome.com.tw/articles/10402644)。
...
bash mutations.sh       # 把功能弄壞 16 次，證明上面那 15 條真的會沒過
```

`mutations.out` 有跑過的結果，最後一列是反向對照：改一句不影響行為的說明文字，verify 應該還是全部通過。沒有那一列的話，「什麼都會讓它沒過」跟「它抓得準」分不開。

寫這份的時候有六條檢查被抓出來是假的，兩條靠突變、四條靠外部審查：

| 原本怎麼寫 | 為什麼是假的 |
|---|---|
| 第 1 條只比 handbook 那一組的數字 | 「只讀前五行」剛好被排在前面的那組蓋過去 |
| 第 13 條用一句字面本來就會贏的問題 | 不管走不走 `--embed` 都通過 |
| 第 4 條把壞行數寫死成 1 | 只記第一個解析錯誤的實作照樣通過 |
| 第 6 條只比同一個檔的兩種餵法 | stdin 壞掉之後固定去讀示範檔，照樣通過 |
| 第 8 條只要求名次大於 1 | 名次固定報第 2 名照樣通過 |
| 第 15 條只 grep 一句警語 | 那是文案檢查不是行為檢查 |

**六條在正常路徑上全都會過。** 現在第 15 條改成驗「走訪跳過任何路徑就要非 0 結束」，那句警語併進第 13 條，用分支差異驗（走 `--embed` 的時候它反而不該出現）。
