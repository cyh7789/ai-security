...
# Day 11 有六條假檢查是同一個形狀：grep 一句話就算過。這裡一條都不准是那樣。
...
if run 3; then
  ...
  HPID=$!
  # 等它真的聽起來，不要用固定秒數：慢的機器上 sleep 1 會變成偶發沒過。
  ...
  # 比整段描述，不是只比 TOTAL：三段內容全換掉但總字元數湊成一樣，只比 TOTAL 會通過。
...
if run 8; then
  ...
  if [ ! -f "${OLD}" ]; then
    ...
    # 斷言要寫 = 0，不能寫 != 2：這一條的重點是「它漏掉卻回通過」，
    # 舊工具哪天改成回 1（有話說）就不是同一件事了，這條就該沒過。
...
# 正是文章那段判斷的內容。哪天有人「修好」讓它們分開，這條會沒過，那時要改的是文章。
if run 12; then
  echo "=== 12 合法命令句與下毒描述被歸進同一類 ==="
  mktree
  OUT=$(node skill-scan.cjs "${TMP}/tree")
  # 要比對「，有命令句」而不是「有命令句」：後者是「沒有命令句」的子字串，
  # 這樣寫的話把判準改壞了它照樣通過。實測過，這條一開始就是這個洞。
...
if run 15; then
  ...
  # 而那正是這支腳本在講的假通過：上一次測試留下的行程讓它變成通過過一次。
...
# 只問第一頁、印出一個數字、回 0，就是這一份從頭到尾在罵的那種假通過，
