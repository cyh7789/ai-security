...
#    全部通過，而其實一條都沒驗到。
# 二、每一列要指定**哪幾條**該沒過，精確比對。只看過不過的話，一個突變可以靠別的檢查沒過
...
# 第一版寫這一列的時候我預期第 5、6、7 條會跟著沒過，結果只有第 1 條沒過。
# （第 7c 條也會沒過，因為它同時在比兩份表差幾行，而這一改就多了差異。）
...
run M7b "把一條描述改一個字，跟官方頁對不上" 沒過 7b
...
run M7d "換描述之前那一輪多出一條不一樣" 沒過 7d

# 底稿的描述改回一致、再去動別列的標題欄。行數照樣是 4，舊版的 DD=4 全部通過。
python3 -c "
import pathlib
...
p.write_text(s)"
run M7c "底稿改在描述以外的欄，diff 行數不變" 沒過 7c
...
run M8b "只給檔名那一輪，改名那發也答對了" 沒過 8b
...
JS
ANTARES_TOOLS_JS="$FIXED" run M9 "靶標換成 execFile 版（洞不見了）" 沒過 9

# 修好那一半的正對照。第一版沒有這個：fixed.js 只要沒真的把指令送出去
# （缺 dig／模組整個 throw），標記檔就不會生出來，於是照樣判通過（外審實測）。
cat > "$FIXED" <<'JS'
module.exports = {
