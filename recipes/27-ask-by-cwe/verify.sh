...
# 恆真，整格空白反而全部通過（外審實測：清空 CWE-639 的依據與備註，這條照樣通過）。
NB=""
...
# 核對表不動，這裡全部通過而後面三條照樣照舊算，判準跟成績就分家了（M1 抓出來的）。
...
# 把 firstdraft 的描述改回一致、再去動別兩列的標題欄，行數照樣是 4，整條假通過（外審實測）。
...
case "$RC" in
  0) ok "$(grep '^通過' /tmp/d27-confirm.$$ | sed 's/^通過：//')" ;;
  2) skip "confirm.sh 回 2（環境不到位，多半是沒有 node）" ;;
  *) bad "confirm.sh 回 ${RC}：$(grep '^沒過' /tmp/d27-confirm.$$ | tr '\n' ' ')" ;;
esac
